...
import Tool from "@/components/HashGenerator";
      ...
      <main className="min-h-screen px-4 py-12">
        <div className="mx-auto max-w-4xl">
          <div className="mb-10 text-center">
            ...
            <h1 className="mb-3 text-4xl font-bold text-white md:text-5xl">Hash Generator</h1>
            <p className="mb-2 text-lg text-text-muted">Generate MD5, SHA-1, SHA-256 and SHA-512 hashes in real time. Integrity verification.</p>
          ...
          <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-3">
            {[
            { icon: "🔐", title: "Four algorithms", desc: "MD5, SHA-1, SHA-256 and SHA-512 in real time." },
            { icon: "✅", title: "Integrity check", desc: "Compare against a known hash to verify content." },
            { icon: "🔒", title: "100% private", desc: "Hashing runs locally with the Web Crypto API." },
            ].map((item) => (
          ...
          <div className="mt-8 rounded-xl border border-border/20 bg-white/3 p-6">
            ...
            <p className="mb-4 text-sm text-text-muted">Add Hash Generator to any page with a simple iframe, or link to it with attribution.</p>
            <div className="mb-3 rounded-lg bg-black/40 p-3">
              <p className="mb-1 text-xs text-text-muted/60">Iframe (plug & play):</p>
              <code className="text-xs text-green-400 break-all">{`<iframe src="${EMBED_URL}" width="100%" height="700" style="border:none;border-radius:12px;" title="Hash Generator — miguelacm.es" loading="lazy"></iframe>`}</code>
            </div>
            <div className="rounded-lg bg-black/40 p-3">
              <p className="mb-1 text-xs text-text-muted/60">Link with attribution (recommended for backlink):</p>
              <code className="text-xs text-green-400 break-all">{`<a href="${SITE_URL}" target="_blank" rel="noopener">Hash Generator — free tool by MACM</a>`}</code>
            </div>
          </div>
        </div>
      </main>
